...
export const createApplication = async (req, res) => {
    ...
    try {
        ...
        const { appId, applicantId, name, email, phone, resume, experience, company, duration, role, education, degree, institute, year, address, city, state, zip, } = req.body;
        ...
        if (!appId || !applicantId || !name) {
            return res.status(400).json({
                message: "Missing required application fields: appId, applicantId, and name are required.",
            });
        }
        // --------------------------------------------------------
...
export const getApplicationByJob = async (req, res) => {
    try {
        ...
        const jobEmployerId = job.createdBy.toString();
...
export const getTotalApplicantsByEmployer = async (req, res) => {
    try {
        const employerId = req.user?.id;
        if (!employerId)
...
export const getTotalJobByEmployer = async (req, res) => {
    try {
        const employerId = req.user?.id;
        if (!employerId) {
            return res.status(401).json({ message: "Unauthorized" });
...
export const getTotalJobViewsByEmployer = async (req, res) => {
    try {
        const employerId = req.user?.id;
